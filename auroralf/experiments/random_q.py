...
from .artifacts import read_toml, resolve_path
...
class Config:
    ...
    def load(cls, path):
        ...
        if not (
            0 <= cfg.z < cfg.z_start
            and cfg.logmass_min < cfg.logmass_max
            and cfg.q_log10_sigma > 0
            and cfg.lookback_myr > 0
        ):
            raise ValueError("invalid physical parameters")
        if not cfg.efficiencies or any(
            not np.isfinite(e) or not 0 < e <= 1 for e in cfg.efficiencies
        ):
...
def first_crossing(time, mass, cooling, logq):
    ...
    if (
        not all(np.all(np.isfinite(x)) for x in (t, m, c, q))
...
def burst_light(time, mass, cooling, logq, ages, kernel, fb, lookback):
    ...
    return dict(
        status=status,
        burst_time_gyr=tb,
        burst_halo_mass_msun=mb,
        age_myr=age,
        popiii_per_efficiency=light,
    )
...
def one_mass(task):
    ...
    for start in range(0, cfg.n_tracks, cfg.track_chunk):
        ...
        p2 = compute_final_ssp_observable_from_sfr_grid(
            t_grid_gyr=time,
        ...
        event = burst_light(
            ...
            mass,
            cooling,
            logq[start : start + cfg.track_chunk],
            a3,
            k3,
