...
from auroralf.constants import PLANCK18_H0_KM_S_MPC, PLANCK18_OMEGA_M, PLANCK18_OMEGA_B, AB_ZEROPOINT_LNU
...
def uv_coefficient(ages, luminosity, duration):
    """Integrate piecewise linear-in-log-age SSP exactly; hold first point to age zero."""
    if duration < ages[0] or duration > ages[-1]:
        raise ValueError('duration outside SSP coverage')
    x = np.r_[ages[ages < duration], duration]
    y = np.interp(np.log(x), np.log(ages), luminosity)
    slope = np.diff(y) / np.diff(np.log(x))
    integral = y[0] * x[0] + np.sum(y[:-1] * np.diff(x) + slope *
        (x[1:] * np.log(x[1:] / x[:-1]) - np.diff(x)))
    return float(integral * 1e6)  # years, not Myr
...
def main():
    config_path = ROOT / 'configs/experiments/visbal_duty.toml'
    ...
    record(config_path); record(Path(__file__))
    samples = []
    for run in (cfg['full_run'], cfg['low_run']):
        path = src / 'data_save/supercomputer_tracks1000_20260907' / run
        manifest = json.loads((path/'manifest.json').read_text())
        assert manifest['status'] == 'complete'
        record(path/'manifest.json')
        for name, digest in manifest['products'].items():
            record(path/name)
            assert hashes[str(path/name)] == digest
    ...
    assert np.all(np.isfinite(p2)) and np.all(w >= 0)
    eligible = (mass >= floor) & (mass <= 2*floor)
    ssp = src/'external_data/ssp_spectra/schaerer2010_pop3/pop3_ge0_logE_500_001_is5.25'
    record(ssp)
    ages, lum = load_popiii_uv_luminosity_table(ssp)
    coefficient = uv_coefficient(ages, lum, cfg['csfr_age_myr'])
    checkx = np.r_[0., np.geomspace(ages[0],cfg['csfr_age_myr'],100000)]
    ...
    cosmo = FlatLambdaCDM(H0=PLANCK18_H0_KM_S_MPC, Om0=PLANCK18_OMEGA_M, Ob0=PLANCK18_OMEGA_B,Tcmb0=0)
    th = (1/cosmo.H(cfg['z'])).to_value(u.yr)
    fb = PLANCK18_OMEGA_B/PLANCK18_OMEGA_M
    original = src/'data_save/popiii_available_tracks_20260907/uvlf.npz'
    record(original)
    old = np.load(original)
    edges = old['bin_edges']; widths = np.diff(edges); centers = (edges[1:]+edges[:-1])/2
    def hist(l, weight):
        return np.histogram(mag(l),bins=edges,weights=weight)[0]/widths
    baseline = hist(p2,w)
    np.testing.assert_allclose(baseline,old['z14.5_baseline_phi'],rtol=1e-9,atol=1e-14)
    curves={'baseline':baseline,'burst10':old['z14.5_eps0.1_phi']}
    rows=[]
    for duty in cfg['duties']:
        ...
        rows.append(dict(duty=duty,effective_time_myr=duty*th/1e6,
            brightest_popiii=float(np.min(mag(l3[(wa>0)]))),
            mean_popiii_emissivity=float(np.sum(wa*l3)),
            density_total_le20=float(np.sum(wi*(mag(p2)<=-20)+wa*(mag(p2+l3)<=-20)))))
    np.testing.assert_allclose([r['mean_popiii_emissivity'] for r in rows],rows[0]['mean_popiii_emissivity'],rtol=1e-12)
    plt.style.use('apj')
    ...
    ax.plot(centers,curves['burst10'],color='.5',ls=':',lw=2,label=r'First-crossing burst (10\%)')
    for duty,color in zip(cfg['duties'],['#a44b20','#008c85','#3973ac']):
        y=curves[f'duty{duty}']
        ax.plot(centers,np.where(y>0,y,np.nan),color=color,lw=2,label=rf'Visbal-style: duty = {100*duty:g}\%')
    ...
    for rel,label,marker,color in [
        ...
        path=src/'external_data/observations/uvlf'/rel; record(path)
        d=np.load(path); assert not np.any(d['is_upper_limit'])
        handles.append(ax.errorbar(d['muverr'],d['phierr'],xerr=d['mag_err'],
            yerr=[d['phi_err_lo'],d['phi_err_up']],fmt=marker,color=color,mfc='white',
            ms=7,capsize=3,label=label,zorder=10))
    ax.set(xlim=(-22,-12),ylim=(3e-8,.2),yscale='log',xlabel=r'$M_{\rm UV}$ [AB mag]',ylabel=r'$\phi$ [Mpc$^{-3}$ mag$^{-1}$]')
    ax.legend(handles=model_handles,loc='upper left',fontsize=10,frameon=False)
    fig.legend(handles=handles,loc='lower left',bbox_to_anchor=(.12,.06),fontsize=10,frameon=False)
    fig.suptitle(r'Visbal-style statistical comparison: $z=14.5$',y=.97,fontsize=16)
    fig.text(.13,.91,r'$f_\star=10\%$; project $M_{\rm cool}$; constant-SFR UV calibration at 100 Myr',fontsize=11)
    fig.text(.13,.025,'Formal duty mixture, not a causal burst model. Same SSP / Pop II baseline; no dust.',fontsize=10)
    out.mkdir(parents=True)
    np.savez_compressed(out/'uvlf.npz',bin_edges=edges,**curves)
    report=dict(config=cfg,mcool_msun=floor,hubble_time_myr=th/1e6,uv_per_sfr=coefficient,
                rows=rows,inputs_sha256=hashes,
                limitations=['Project threshold/cosmology/logE, not exact paper reproduction.',
    ...
    print(json.dumps({k:v for k,v in report.items() if k not in ('inputs_sha256','config')},indent=2))
