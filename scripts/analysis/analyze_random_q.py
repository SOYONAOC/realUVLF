...
def observation_specs(z):
    ...
    if z == 14.5:
        return [
            ('redshift_14/whitler25_jades_z14p3.npz','Whitler+25: JADES, median z=14.3','o','#333333',True),
            ('redshift_15/donnan24_primer_z14p5.npz','Donnan+24: PRIMER (tentative)','s','#945399',True),
            ('redshift_15/naidu26_mom_jades_spectroscopic_z14p5.npz',r'Naidu+26: MoM+JADES spec., $14<z<15$','D','#207798',True)]
    if z == 12.5:
        ...
        return [
            ...
            ('redshift_12p5/harikane23_uvlf_z12.npz',r'Harikane+23: $z\simeq12$','D','#207798',False)]
...
def summarize_run(path, edges):
    ...
    for name, h in manifest['products'].items():
    ...
    for key in ('burst_time_gyr','burst_halo_mass_msun','age_myr'):
        if not (np.isfinite(samples[key][status==1]).all() and np.isnan(samples[key][status!=1]).all()):
            raise ValueError('event coordinates '+key)
    if np.any(samples['popiii_per_efficiency'][status!=1] != 0):
        raise ValueError('non-event light')
    p2 = samples['popii']; p3 = samples['popiii_per_efficiency']
    curves = {'baseline':p2, 'fixed_q1_eps0.1':p2+.1*samples['fixed_q1_per_efficiency']}
    curves.update({f'eps{e:g}':p2+e*p3 for e in c['efficiencies']})
    width = np.diff(edges)
    outputs = {}; summary = []
    for name, l in curves.items():
        mag = uv_luminosity_to_muv(l)
        ...
        for cut in (-18., -19., -20., -21., -22.):
            ...
            row['thresholds'].append(dict(cut=cut, total=float(contrib.sum()), added=float(added.sum()),
                total_se=float(np.sqrt(n*np.var(contrib,ddof=1))), added_se=float(np.sqrt(n*np.var(added,ddof=1))),
                added_tracks=int(np.sum(selected & ~baseline)),
                added_from_q_gt100=float(((selected & ~baseline & (samples['logq']>2)).sum(axis=1)*w).sum()) if name.startswith('eps') else 0.))
        if name.startswith('eps'):
            e = float(name[3:])
            row['brightest_popiii'] = float(np.nanmin(uv_luminosity_to_muv(e*p3))) if np.any(p3>0) else None
        summary.append(row)
    logq=samples['logq']
    diag=dict(logq_mean=float(logq.mean()),logq_std=float(logq.std()),
        q_central_fraction=float(np.mean((logq>=-1)&(logq<=2))), q_high_fraction=float(np.mean(logq>2)),
        not_triggered_fraction=float(np.mean(status==0)), left_censored_fraction=float(np.mean(status==2)),
        resolved_fraction=float(np.mean(status==1)),uv_active_fraction=float(np.mean(p3>0)))
...
def main():
    ...
    data={'bin_edges':edges}; rows=[]
    for key in loaded[0][1]:
        stack=np.array([x[1][key][0] for x in loaded])
        data[key]=stack.mean(axis=0)
        data[key+'_cluster_se']=np.sqrt(np.sum([x[1][key][1]**2 for x in loaded],axis=0))/len(loaded)
        data[key+'_batch_se']=stack.std(axis=0,ddof=1)/np.sqrt(len(loaded)) if len(loaded)>1 else np.full(len(edges)-1,np.nan)
    ...
    fig.subplots_adjust(left=.13,right=.97,top=.88,bottom=.28)
    x=(edges[1:]+edges[:-1])/2
    for key,color,label,ls in [('baseline','black','Pop II only','-'),
            ('fixed_q1_eps0.1','.5',r'Fixed $q=1$, $\epsilon_b=10\%$',':')]+[
            (f'eps{e:g}',color,rf'Random $q$: $\epsilon_b={100*e:g}\%$',ls)
            for e,color,ls in zip(cfg['efficiencies'],['#9564bf','#3973ac','#008c85','#d75a00','#874e21'],[':', '--','-', '-.','-'])]:
        y=data[key]; se=data[key+'_cluster_se']; good=(y>0)&(data[key+'_counts']>=8)
        ax.plot(x,np.where(good,y,np.nan),color=color,ls=ls,lw=1.8,label=label)
        if key.startswith('eps'):
            ax.fill_between(x,np.where(good & (y>se),y-se,np.nan),np.where(good,y+se,np.nan),color=color,alpha=.12)
    mh,_=ax.get_legend_handles_labels(); obs=[]; sources={}
    for name,label,marker,color,has_limit_column in observation_specs(cfg['z']):
        f=a.observations/name; sources[str(f)]=digest(f)
        with np.load(f) as d:
            ...
            obs.append(ax.errorbar(d['muverr'],d['phierr'],xerr=d['mag_err'],yerr=[d['phi_err_lo'],d['phi_err_up']],
                fmt=marker,color=color,mfc='white',ms=7,capsize=3,label=label,zorder=10))
    visible=(x>=-24)&(x<=-12)
    top=max(np.max(data[k][visible]) for k in loaded[0][1])
    ax.set(xlim=(-24,-12),ylim=(3e-9,max(.02,top*1.5)),yscale='log',
        xlabel=r'$M_{\rm UV}$ [AB mag]',ylabel=r'$\phi$ [Mpc$^{-3}$ mag$^{-1}$]')
    ax.legend(handles=mh,loc='upper left',frameon=False,fontsize=10)
    fig.legend(handles=obs,loc='lower left',bbox_to_anchor=(.12,.065),frameon=False,fontsize=10)
    fig.suptitle(rf'Random first-crossing Pop III burst: $z={cfg["z"]:g}$',y=.975,fontsize=16)
    fig.text(.13,.915,r'$\log_{10}q\sim\mathcal{N}(0.5,1.5^2)$; one draw per track; no tail truncation',fontsize=11)
    fig.text(.13,.035,'Pop II stellar 1600A + Pop III total 1500A; no dust. No pristine/enrichment closure.',fontsize=9)
    fig.text(.13,.012,'Shading: mass-cluster MC SE; not cosmic variance or model uncertainty. No curve smoothing.',fontsize=9)
    a.output.mkdir(parents=True)
    np.savez_compressed(a.output/'uvlf.npz',**data)
    report=dict(config=cfg, runs=[str(p) for p in a.runs], rows=rows, diagnostics=[x[3] for x in loaded],
        per_batch_rows=[x[2] for x in loaded],
        manifests_sha256={str(p/'manifest.json'):digest(p/'manifest.json') for p in a.runs},
        observations_sha256=sources,analysis_sha256=digest(Path(__file__)),
        limitations=['User-assumed independent q distribution, not calibrated physics.',
    ...
    fig.savefig(a.output/'uvlf.png',dpi=170);fig.savefig(a.output/'uvlf.pdf');plt.close(fig)
