...
import numpy as np
from scripts.experiments.random_q_burst import Config, initialize_worker, one_mass
...
def check_compute_site(site, env):
    ...
    if site == 'fat2' and partition == 'fat' and env.get('SLURM_JOB_NODELIST') == 'fat2':
        return
    if (site == 'node123' and partition == 'cpu'
            and env.get('SLURM_JOB_NODELIST') in ('node[1-3]', 'node1,node2,node3')
            and env.get('SLURMD_NODENAME') in ('node1','node2','node3')):
        return
    raise RuntimeError('Allocation does not match explicit compute site; all debug partitions are forbidden')
...
def main():
    ...
    if a.validate_only:
        ...
        return
    if not os.environ.get('SLURM_JOB_ID') or int(os.environ.get('SLURM_CPUS_PER_TASK', 0)) < cfg.workers:
        raise RuntimeError('SLURM allocation required')
    stage.mkdir(parents=True)
    sources = list((ROOT/'auroralf').rglob('*.py')) + [Path(__file__), ROOT/'scripts/experiments/random_q_burst.py', a.config.resolve(), ROOT/'pyproject.toml', ROOT/'uv.lock']
    hashes = {str(s.relative_to(ROOT)): digest(s) for s in sources}
    inputs = {s: digest(s) for s in (cfg.popii_ssp, cfg.popiii_ssp)}
    manifest = dict(config=asdict(cfg), code_sha256=hashes, input_sha256=inputs,
        job_id=os.environ['SLURM_JOB_ID'], compute_site=a.compute_site,
        partition=os.environ.get('SLURM_JOB_PARTITION'), node_list=os.environ.get('SLURM_JOB_NODELIST'),
    ...
    if a.slurm_shards > 1:
        manifest['sharding'] = dict(index=shard_index, count=a.slurm_shards,
            global_n_mass=cfg.n_mass, weight_normalization='global_n_mass; sum disjoint shards, never average')
    import tarfile
    ...
    try:
        ...
        hmf = prepare_reed07_hmf_interpolator(log10_halo_mass_min_msun=cfg.logmass_min, log10_halo_mass_max_msun=cfg.logmass_max,
                                            z_obs=cfg.z, cosmology=Cosmology())
        weight = (cfg.logmass_max-cfg.logmass_min)*np.log(10)*mass*hmf.evaluate(mass)/cfg.n_mass/cfg.n_tracks
        samples = {}
        start = time.monotonic()
        with ProcessPoolExecutor(max_workers=cfg.workers, mp_context=mp.get_context('spawn'),
                                 initializer=initialize_worker, initargs=(cfg,)) as pool:
            ...
            for first in range(0, len(indices), 2*cfg.workers):
                ...
                print(f'shard={shard_index} mass_done={done}/{len(indices)} elapsed_s={time.monotonic()-start:.1f}', flush=True)
        ...
        if any(digest(ROOT/k) != h for k,h in hashes.items()) or any(digest(k) != h for k,h in inputs.items()):
